...
type AccountSettingsProps = {
  ...
  // Avisa o container (ex.: a navbar do cofre) quando nome/foto mudam, para
  // refletir na hora sem precisar recarregar.
...
const NAV: { tab: Tab; icon: typeof User; label: string }[] = [
  ...
  { tab: "segurança", icon: Lock, label: "Segurança" },
  { tab: "sessões", icon: Monitor, label: "Sessões" },
  { tab: "conexões", icon: Globe, label: "Conexões" },
  { tab: "preferências", icon: Settings, label: "Preferências" },
  { tab: "conta", icon: Settings, label: "Conta" },
];
  ...
  const content = (
    ...
    >
      ...
      <nav className="account-settings-nav flex w-52 shrink-0 flex-col border-r border-[color:var(--border)] p-4 pt-5">
        ...
        {NAV.map(({ tab: id, icon: Icon, label }) => (
          <button
            key={id}
            type="button"
            onClick={() => setTab(id)}
...
function PerfilTab({
  ...
  user: SessionUser | null;
  onProfileChange?: AccountSettingsProps["onProfileChange"];
}) {
  const { t } = useTranslation();
  ...
  async function saveName(e: FormEvent) {
    ...
    try {
      ...
      setProfile((current) =>
        current ? { ...current, fullName: nextFullName } : current,
      );
      onProfileChange?.({
